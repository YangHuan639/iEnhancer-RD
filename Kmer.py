...
def getDNA(filename):
    ...
    for line in fr.readlines():
        line = line.strip('\n')
        if(line[0]!='>'):
            str=line.upper()#将DNA序列中核苷酸小写字符转换成大写字符
            # str = [i.upper() for i in line]#将DNA序列中核苷酸小写字符转换成大写字符
            SSeq.append(str)
    return SSeq

def f1_kmer(SSeq):#获取1-mer的列表值
    num = 0
    all_freq1 = []
    Nuc1=[]
    for firchCh in ['A', 'C', 'G', 'T']:
        ...
        Nuc1.append(one_mer)
    print(len(Nuc1))
    for seq in SSeq:#遍历每一条DNA序列
        f1 = []
        for N in Nuc1:#将Nuc1中的每一个核苷酸与一条DNA序列中的每一个核苷酸匹配
...
def f2_kmer(SSeq):
    ...
    for seq in SSeq:#遍历每一条DNA序列
        f2 = []
        for N in Nuc2:#将Nuc1中的每一个核苷酸与一条DNA序列中的每一个核苷酸匹配
...
def f3_kmer(SSeq):
    ...
    for seq in SSeq:#遍历每一条DNA序列
        f3 = []
        for N in Nuc3:#将Nuc1中的每一个核苷酸与一条DNA序列中的每一个核苷酸匹配
...
def f4_kmer(SSeq):
    ...
    for seq in SSeq:#遍历每一条DNA序列
        f4 = []
        for N in Nuc4:#将Nuc1中的每一个核苷酸与一条DNA序列中的每一个核苷酸匹配
...
def f5_kmer(SSeq):
    ...
    for seq in SSeq:  # 遍历每一条DNA序列
        f5 = []
        for N in Nuc5:  # 将Nuc1中的每一个核苷酸与一条DNA序列中的每一个核苷酸匹配
...
def load_data(filename1, filename2):
    Dnaseq1 = getDNA(filename1)  # 获取DNA序列
    Dnaseq2 = getDNA(filename2)

    data1_np = np.array(Dnaseq1)  # 将列表转换为numpy格式
    data2_np = np.array(Dnaseq2)
    data_12 = np.hstack([data1_np, data2_np])  # 将两个numpy数据垂直连接

    y1 = label_Pos(Dnaseq1)
    y2 = label_Neg(Dnaseq2)  # 负样本标签0
    y1_np = np.array(y1)
    y2_np = np.array(y2)
...
def Independence_test(Data12, Label12, Data34, Label34):
    ...
    print("独立集测试",acc, sens, spec, mcc, auc)

def parameters_select(filename1,filename2):
    best_ACC = 0.0
    i = 0
    for k in range(1, 6):  # lambda,python的关键字，故不能用它来定义变量
        i = i + 1
        Sseq, Label = load_data(filename1, filename2)  # 获取DNA序列，以及生成label
        Train_val = Data_process(Sseq, k)  # 获取序列特征
    ...
    print("二层Best parameters:{}".format(best_parameter))
    return best_parameter

filename1="D:\\code-summary\\iEnhancer\\data\\fasta\\StrongEnhancer.txt"
filename2="D:\\code-summary\\iEnhancer\\data\\fasta\\WeakEnhancer.txt"
filename3="D:\\code-summary\\iEnhancer\\data\\fasta\\Independence_test\\test\\StrongEnhancer.txt"
filename4="D:\\code-summary\\iEnhancer\\data\\fasta\\Independence_test\\test\\WeakEnhancer.txt"
...
"""
以下最佳参数是验证集上的最佳参数，验证集用于调参寻找最佳参数，但是验证集上的参数在测试集上可能不是最佳的参数，
因为测试集数据只能用于测试以评估模型的泛化性能，所以不能选测试集上最好的参数。

一层预测
Best score:0.754019929019929
Best parameters:{'k': 4}
0.745 0.73 0.76 0.4902206489492161 0.745

二层预测
Best score:0.63
Best parameters:{'k': 1}
独立集测试 0.565 0.59 0.54 0.1301628053236573 0.565
